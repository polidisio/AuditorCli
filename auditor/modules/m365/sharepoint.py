    ...
    try:
        ...
        if sharing_cap == "externalUserAndGuestSharing" and anon_link_expiry == 0:
            findings.append(Finding(
                ...
                remediation="Set expiration on anonymous links: SharePoint Admin Center → Sharing → set 'These links must expire within this many days'.",
            ))
        ...
        if legacy_auth is True:
            findings.append(Finding(
                ...
                remediation="Disable legacy auth: SharePoint Admin Center → Settings → Legacy authentication → Off.",
            ))
    ...
    try:
        ...
        if anon_sites:
            findings.append(Finding(
                ...
                remediation="Review each site in SharePoint Admin Center. Restrict sharing to authenticated users minimum.",
            ))
    ...
    try:
        ...
        if od_link_type == "externalUserAndGuestSharing":
            findings.append(Finding(
                ...
                remediation="Restrict OneDrive sharing to authenticated external users or org-only.",
            ))
    except Exception:
        pass
